...
- 静态修补通过系统文件选择器的 URI 授权读写，不申请存储权限，也不需要 Root 或 LSPosed。
- 运行时适配只需要 LSPosed 启用模块并勾选相应作用域。
- Root 仅用于用户主动执行“重启作用域”；应用启动时不再探测 `su`。
- 包可见性只声明模块的固定作用域列表，不再申请 `QUERY_ALL_PACKAGES`。
